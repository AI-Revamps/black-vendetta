...
require 'config.php';
$stmt = pdo_query("SELECT * FROM `users` WHERE `login` = ?", [$_SESSION['login']]);
$data = $stmt->fetch();
if(! check_login()) {
...
if (isset($_POST['addnew'])) {
    $title = $_POST['title'] ?? '';
    $text  = $_POST['text'] ?? '';
    if ($title === '') { echo "Je hebt geen titel opgegeven."; exit; }
    if ($text === '') { echo "Je hebt geen tekst opgegeven."; exit; }
    $time = time();
    pdo_query(
...
if (isset($_POST['edit'])) {
    ...
    if ($text === '') { echo "Je hebt geen tekst opgegeven."; exit; }
    $time = time();
    if (($_POST['t'] ?? '0') != 1) { $time = $_POST['time'] ?? $time; }
    $id = $_POST['id'] ?? '';
    pdo_query(
...
elseif (isset($_GET['d'])) {
  $id = $_GET['d'];
  $stmt = pdo_query("SELECT * FROM `news` WHERE `id` = ?", [$id]);
  $news = $stmt->fetch();
...
<table width="100%">
  ...
  <tr>
    <td class="mainTxt">**********<br>
	  <?php
        $pp = 10;
        $start = (isset($_GET['p']) && $_GET['p'] >= 0) ? (int)$_GET['p'] * $pp : 0;
        $rows = pdo_query("SELECT COUNT(*) AS c FROM `news`")->fetch()->c;
        if($start > $rows){
            $start = 0;
        }
        $stmt = pdo_query("SELECT * FROM `news` ORDER BY time DESC LIMIT $start,$pp");
        while($news = $stmt->fetch()){
            $id = $news->id;
            $time = date('d/m/Y', $news->time);
            $title = htmlspecialchars($news->title);
            $text  = htmlspecialchars($news->text);
...
<table width="100%">
  ...
  <tr>
    <td class="mainTxt">**********<br>
	  <?php
        $pp = 10;
        $start = (isset($_GET['e']) && $_GET['e'] >= 0) ? (int)$_GET['e'] * $pp : 0;
        $rows = pdo_query("SELECT COUNT(*) AS c FROM `news`")->fetch()->c;
        if($start > $rows){
            $start = 0;
        }
