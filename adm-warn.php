...
$user = $_GET['x'] ?? '';
if ($user !== '') {
    $msg = 'Er is meer dan 1 account op je ip gevonden, dit is tegen de regels. '
        . 'Stuur een bericht naar een admin om je tweede account te rechtvaardigen, '
        . 'anders is de kans groot dat een van je accounts verwijderd wordt.'
        . '(Verzonden door ' . $data->login . ')';
    pdo_query(
        'INSERT INTO `messages`(`time`,`from`,`to`,`subject`,`message`) VALUES (NOW(),\'Notificatie\', ?, \'Waarschuwing\', ?)',
        [$user, $msg]
    );
