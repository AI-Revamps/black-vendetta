...
$msg = $message[$msgnum];
if ($kkind == 1) {
    echo "Je hebt &euro;$ammount Gestolen.";
    pdo_query(
...
pdo_query(
    "UPDATE `users` SET `xp`=`xp`+1,`crime`=FROM_UNIXTIME(?),`nrofcrime`=`nrofcrime`+1 WHERE `login`=?",
    [$crimetijd, $data->login]
);
} 
...
if ($kpuber == 1) {
    ...
    if ($msgnum == 5) {
        pdo_query(
            "UPDATE `users` SET `health`=`health`-2 WHERE `login`=?",
            [$data->login]
        );
    }
}
...
if ($kjuwelier == 1) {
    ...
    pdo_query(
        "UPDATE `users` SET `health`=`100` WHERE `login`=?",
        [$data->login]
    );
}
pdo_query(
...
if ($kbar == 1) {
    echo "$schietmsg";
    pdo_query(
        "UPDATE `users` SET `se`=`se`+0.1 WHERE `login`=?",
        [$data->login]
    );
} else {
    echo "$msg";
    ...
    if ($msgnum == 5) {
        pdo_query(
            "UPDATE `users` SET `health`=`health`-1 WHERE `login`=?",
            [$data->login]
        );
    }
}
...
$msg = $message[$msgnum];
$me = pdo_query("SELECT * FROM `users` WHERE `login`!= ? AND `stad` = ? AND `status`='levend' AND `level`!='255' AND `level`!='1000' ORDER BY `zak` DESC LIMIT 0,1", [$data->login, $data->stad]);
$mem = $me->fetch();
...
elseif ($kmember == 1) {
    echo "Je hebt &euro;$ammount gestolen van $mem->login";
    pdo_query("UPDATE `users` SET `zak`=`zak`-? WHERE `login`=?", [$ammount, $mem->login]);
    pdo_query("UPDATE `users` SET `zak`=`zak`+? WHERE `login`=?", [$ammount, $data->login]);
    pdo_query(
        "INSERT INTO `messages`(`time`,`from`,`to`,`subject`,`message`) values(NOW(),'Notificatie',?,'Zakkenroller',?)",
        [$mem->login, "$data->login heeft &euro;$ammount uit je zak gestolen."]
    );
} else {
    echo "$msg";
    if ($msgnum == 2) {
        pdo_query("INSERT INTO `jail`(`login`,`boete`,`stad`,`famillie`,`time`) VALUES(?,?,?,?,FROM_UNIXTIME(?))", [$data->login, $boete, $data->stad, $famillie, $jailtime]);
    } elseif ($msgnum == 6) {
        pdo_query("UPDATE `users` SET `health`=`health`-1 WHERE `login`=?", [$data->login]);
    }
}
pdo_query("UPDATE `users` SET `crime`=FROM_UNIXTIME(?),`nrofcrime`=`nrofcrime`+1 WHERE `login`=?", [$crimetijd, $data->login]);
}
exit;
}
print "
